...
test('shows an empty state when the server has no visible channel', async () => {
  ...
  const { getByTestId } = await render(
    <ServerChannelsScreen server={SERVER} onOpenChannel={jest.fn()} onOpenForum={jest.fn()} onBack={jest.fn()} />,
  );
  await waitFor(() => expect(getByTestId('server-channels-empty')).toBeTruthy());
});

test('tapping a text or announcement channel calls onOpenChannel, not onOpenForum', async () => {
  mockedServers.loadServerChannels.mockResolvedValueOnce([
  ...
  const { getByTestId } = await render(
    <ServerChannelsScreen server={SERVER} onOpenChannel={onOpenChannel} onOpenForum={onOpenForum} onBack={jest.fn()} />,
  );
...
test('tapping a forum channel calls onOpenForum, not onOpenChannel', async () => {
  mockedServers.loadServerChannels.mockResolvedValueOnce([
...
test('the back button calls onBack', async () => {
  ...
  const { getByTestId } = await render(
    <ServerChannelsScreen server={SERVER} onOpenChannel={jest.fn()} onOpenForum={jest.fn()} onBack={onBack} />,
  );
